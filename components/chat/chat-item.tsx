...
export const ChatItem = memo(({
  ...
  return (
    <div className="relative group flex items-center hover:bg-black/5 p-4 transition w-full">
      ...
      <button type="button" onClick={onMemberClick} aria-label={`Посмотреть профиль пользователя ${member.profile.name}`} className="cursor-pointer hover:drop-shadow-md transition focus:outline-none focus:ring-2 focus:ring-zinc-500 focus:ring-offset-2">
          <UserAvatar src={member.profile.imageUrl} />
        </button>
        <div className="flex flex-col w-full">
          <div className="flex items-center gap-x-2">
            <div className="flex items-center">
            <button type="button" onClick={onMemberClick} aria-label={`Посмотреть профиль пользователя ${member.profile.name}`} className="font-semibold text-sm hover:underline cursor-pointer focus:outline-none focus:ring-2 focus:ring-zinc-500 focus:ring-offset-2">
                {member.profile.name}
              </button>

...
## 2024-05-27 - Incomplete URL Validation in ImageUrlSchema
**Vulnerability:** The `ImageUrlSchema` in `/app/api/servers/route.ts` and `/app/api/servers/[serverId]/route.ts` validated user input for an image URL with just `.startsWith("data:image/") || /^https?:\/\//.test(v)`. This weak regex validation does not ensure the string is a structurally sound URL and could be bypassed (e.g., missing the case insensitive `/i` flag meant valid uppercase schemes were rejected, but conversely, malformed URLs passing the start string check were accepted).
**Learning:** Checking the prefix of a string is insufficient for validating URLs. An attacker could provide a string that begins with a valid protocol but is otherwise malformed or points to unexpected resources (SSRF risk).
**Prevention:** When validating inputs that accept both HTTP(S) URLs and data URIs, use `z.string().refine()` to combine `v.startsWith('data:image/')` with `z.string().url().safeParse(v).success && /^https?:\/\//i.test(v)`. This enforces strict URL structure and safe protocols, preventing SSRF and Stored XSS bypasses that native schema validation might miss when accommodating multiple formats.

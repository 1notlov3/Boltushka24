...
## 2024-05-27 - Unvalidated Query Parameters in Socket API Routes
**Vulnerability:** Socket API endpoints (e.g., `pages/api/socket/messages/[messageId].ts`) accessed `req.query` parameters directly and passed them to Prisma queries. Passing invalid formatted strings (non-UUIDs) to database ID fields causes unhandled 500 Server Errors.
**Learning:** Next.js Pages Router API routes expose route variables in `req.query`. Just like Next.js App Router route `params`, these must be explicitly validated before use in database interactions to prevent errors and ensure data integrity.
**Prevention:** Always validate URL search parameters (`req.query`) in Next.js API routes using schema validators like Zod (e.g. `z.string().uuid()`) before executing database queries to enforce defense-in-depth.

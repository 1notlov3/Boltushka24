...
## 2024-05-27 - Unvalidated Socket API Route Parameters
**Vulnerability:** Socket API routes (`pages/api/socket/*`) and `app/api/ratings/route.ts` were using `req.query` parameters (like `conversationId`, `messageId`, `serverId`) directly in Prisma queries without validating that they were valid UUIDs.
**Learning:** Even internal API routes and socket handlers must explicitly validate all input parameters for defense-in-depth, data integrity, and clear client errors.
**Prevention:** Always define Zod schemas for query parameters in Next.js Pages API routes and validate them using `.uuid()` before passing them to the database layer.

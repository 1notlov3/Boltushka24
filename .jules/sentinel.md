...
## 2024-05-24 - Unhandled 500 Server Errors due to missing UUID Validation
**Vulnerability:** Socket API endpoints in `pages/api/socket/` were vulnerable to unhandled 500 Server Errors when passed malformed IDs.
**Learning:** Prisma throws 500 errors if string IDs do not match the expected UUID format for UUID columns. `req.query` parameters were missing UUID format validation.
**Prevention:** Always validate `req.query` parameters in Next.js API routes using Zod schemas (e.g., `z.string().uuid()`) before executing Prisma queries, and ensure duplicated schemas across related endpoints are all updated.

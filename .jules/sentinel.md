...
## 2024-05-27 - Missing UUID Validation in Socket Direct Messages
**Vulnerability:** The `/api/socket/direct-messages` endpoint accepted `conversationId` query parameter without verifying it was a valid UUID, relying on implicit database behavior.
**Learning:** Inconsistent validation creates gaps; while some routes (`/api/socket/messages`) validated IDs, others didn't. Copy-pasting patterns often misses validation steps.
**Prevention:** Enforce a strict "schema-first" approach where every API handler defines a `querySchema` alongside `bodySchema` before any logic is executed.

...
## 2024-05-27 - Unvalidated Query Parameters in Socket APIs
**Vulnerability:** Socket API routes (specifically `pages/api/socket/direct-messages/index.ts`) were using `req.query.conversationId` directly in database queries without validation.
**Learning:** Even though Next.js API routes provide query parameters, they are `string | string[]` and untrusted. Casting them `as string` without validation is risky.
**Prevention:** Define strict Zod schemas for all query parameters (`req.query`) and validate them (e.g., `z.string().uuid()`) before use, returning 400 Bad Request on failure.

...
## 2024-05-27 - Stored XSS via Image URL in Server API
**Vulnerability:** The server creation and update APIs (`app/api/servers/route.ts` and `app/api/servers/[serverId]/route.ts`) accepted image URLs using a weak regular expression (`/^https?:\/\//.test(v)`). This regex only checks the beginning of the string and does not validate the overall structure of the URL, which could lead to SSRF or XSS bypasses.
**Learning:** When validating inputs that accept both HTTP(S) URLs and data URIs (e.g., image URLs), relying solely on `startsWith` or weak regular expressions is insufficient. Native `z.string().url()` validation alone is also insufficient because it allows dangerous protocols like `javascript:`.
**Prevention:** Use `z.string().refine()` to combine `v.startsWith('data:image/')` with `z.string().url().safeParse(v).success && /^https?:\/\//i.test(v)`. This enforces strict URL structure and safe protocols, preventing SSRF and Stored XSS bypasses that native schema validation might miss when accommodating multiple formats.

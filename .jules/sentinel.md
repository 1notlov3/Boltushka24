...
## 2024-05-27 - Unvalidated Route Parameters in Socket APIs
**Vulnerability:** Socket API routes under `pages/api/socket/` were extracting `req.query` parameters (like `serverId`, `channelId`, `conversationId`, `messageId`) and passing them directly to Prisma queries without prior format validation, similar to earlier findings in standard APIs.
**Learning:** The lack of strict parameter validation in legacy `req.query` inputs introduces a vulnerability to unhandled 500 errors and circumvents defense-in-depth principles. Prisma exceptions on malformed UUID strings can inadvertently leak execution paths or cause resource exhaustion.
**Prevention:** Extend strict Zod schema validation (e.g., `z.string().uuid()`) consistently to `req.query` across all Pages API routes before invoking database queries.

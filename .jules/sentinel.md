...
## 2024-05-27 - Unhandled 500 Errors in Pages Router
**Vulnerability:** Socket endpoints under `pages/api/socket/` were passing unvalidated `req.query` parameters to Prisma queries.
**Learning:** When handling URL parameters mapped to database UUID fields, Prisma can throw unhandled 500 Server Errors if passed malformed strings.
**Prevention:** Always validate `req.query` parameters in Next.js Pages Router API routes using Zod schemas (like `z.string().uuid()`) before using them in database queries to prevent unhandled 500 errors and ensure data integrity.

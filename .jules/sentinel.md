...
## 2024-05-27 - Incomplete Validation for Mixed URL Formats
**Vulnerability:** The `imageUrl` validation in server APIs (e.g., `app/api/servers/route.ts`) accepted URLs strictly based on `.startsWith("data:image/")` or `/^https?:\/\//.test(v)`. This allowed structurally malformed inputs bypassing standard URL formatting checks or allowing mixed-protocol injection vectors like SSRF or Stored XSS where the protocol string exists but the format is wrong.
**Learning:** When validating inputs that accept both HTTP(S) URLs and data URIs (e.g., image URLs), native schema URL validators might miss safe protocol enforcements while simple regexes might ignore full structural integrity.
**Prevention:** Always use `z.string().refine()` to combine `v.startsWith('data:image/')` with `z.string().url().safeParse(v).success && /^https?:\/\//i.test(v)`. This enforces strict URL structure and safe protocols, preventing SSRF and Stored XSS bypasses that native schema validation might miss when accommodating multiple formats.

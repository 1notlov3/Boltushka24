...
## 2024-05-27 - SSRF/XSS Bypass via Missing URL Structure Validation
**Vulnerability:** The `ImageUrlSchema` validation in server and initial modal APIs only checked for an `http/https` prefix using regex (`/^https?:\/\//.test(v)`), omitting structural validation. This allows attackers to bypass validation with malformed strings (e.g., `https://` followed by malicious scripts) that could be interpreted unsafely by frontend components or downstream systems, leading to Stored XSS or SSRF.
**Learning:** Checking the protocol prefix with regex is insufficient if the rest of the string is not a structurally valid URL. When accommodating multiple formats (like `data:image/` URIs alongside standard URLs), native Zod URL validation (`z.string().url()`) might be bypassed if not explicitly combined with the protocol check.
**Prevention:** Always combine explicit protocol allowlist regex (e.g., `/^https?:\/\//i.test(v)`) with strict URL structure validation (e.g., `z.string().url().safeParse(v).success`) when validating user-provided URLs.

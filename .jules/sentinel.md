...
## 2026-03-10 - Prevent SSRF/Stored XSS in URL Validation
**Vulnerability:** The `ImageUrlSchema` allowed any HTTP(S) URL and native URL objects which could bypass weak matching when using simple string checks like `/^https?:\/\//.test(v)`, potentially leading to Stored XSS or Server-Side Request Forgery if a crafted URL bypasses the client or naive regex validation but gets serialized improperly.
**Learning:** Simple string matching for URLs is insufficient for robust security because it might allow unexpected protocols, credentials, or malformed queries that bypass superficial checks. Combining strict Zod URL parsing with protocol regex validation provides a stronger defense.
**Prevention:** When validating HTTP(S) URLs alongside data URIs in schemas, combine `z.string().url().safeParse(v).success` with strict protocol checking like `/^https?:\/\//i.test(v)`.

...
## 2024-05-27 - Malformed URL Bypass in Custom Image Validation
**Vulnerability:** The `imageUrl` field in server creation/update APIs (`app/api/servers/route.ts` and `app/api/servers/[serverId]/route.ts`) accepted URLs strictly using a regex `/^https?:\/\//` without `z.string().url()`. This allows malformed URLs such as `http://   ` or URLs that do not resolve, leading to possible errors or SSRF in edge cases.
**Learning:** While regex is good for ensuring the protocol is correct and mitigating Javascript protocol XSS, a combination of `z.string().url().safeParse(v).success` alongside the protocol regex (`/^https?:\/\//i.test(v)`) provides proper defense-in-depth, preventing malformed URL structural bypasses that native regex might miss.
**Prevention:** When validating inputs that accept both HTTP(S) URLs and data URIs (e.g., image URLs), use `z.string().refine()` to combine `v.startsWith('data:image/')` with `z.string().url().safeParse(v).success && /^https?:\/\//i.test(v)`.

...
## 2024-05-27 - Unvalidated Route Parameters in Pages Router
**Vulnerability:** Socket API routes under `pages/api/socket` used route parameters (`req.query`) directly without structural or type validation, making them vulnerable to malformed inputs or type confusion errors.
**Learning:** In Next.js Pages API routes, `req.query` values can be `string | string[]`. Passing unvalidated query objects directly to database functions could cause unhandled Prisma validation errors or logic bypasses if the system expects a single valid string UUID.
**Prevention:** Always validate `req.query` parameters against a strict Zod schema (e.g., `z.string().uuid()`) and extract only the validated properties using `safeParse` before passing them to Prisma. Ensure a 400 response is returned if validation fails.
